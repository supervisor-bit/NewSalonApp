<?php require_once 'auth.php';
 
// edit_visit.php

require 'db.php';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $client_id = (int)$_POST['client_id'];
    $visit_id = (int)$_POST['visit_id'];
    $visit_date = $_POST['visit_date'];
    
    // Služby (přidáno)
    $s_metal_detox = isset($_POST['s_metal_detox']) ? 1 : 0;
    $s_trim = isset($_POST['s_trim']) ? 1 : 0;
    $s_blow = isset($_POST['s_blow']) ? 1 : 0;
    $s_curl = isset($_POST['s_curl']) ? 1 : 0;
    $s_iron = isset($_POST['s_iron']) ? 1 : 0;
    $build_bowl_storage_name = static function ($bowl_name, $mix_ratio = '') {
        $clean_name = trim((string)$bowl_name);
        $clean_ratio = trim((string)$mix_ratio);
        if ($clean_name === '') {
            $clean_name = 'Miska';
        }
        return $clean_ratio !== '' ? ($clean_name . ' || ' . $clean_ratio) : $clean_name;
    };

    try {
        $pdo->beginTransaction();
        
        // Price and Note are managed via the POS Module checkout.php, never overwrite them softly here.
        $stmt = $pdo->prepare("UPDATE visits SET visit_date = ?, s_metal_detox = ?, s_trim = ?, s_blow = ?, s_curl = ?, s_iron = ? WHERE id = ?");
        $stmt->execute([$visit_date, $s_metal_detox, $s_trim, $s_blow, $s_curl, $s_iron, $visit_id]);
        
        $pdo->prepare("DELETE FROM formulas WHERE visit_id = ?")->execute([$visit_id]);
        $pdo->prepare("DELETE FROM visit_products WHERE visit_id = ?")->execute([$visit_id]);
        
        $bowl_names = $_POST['bowl_names'] ?? [];
        $f_stmt = $pdo->prepare("INSERT INTO formulas (visit_id, material_id, amount_g, bowl_name) VALUES (?, ?, ?, ?)");
        
        foreach ($bowl_names as $bIndex => $bName) {
            if (empty($bName)) continue;
            $mixRatio = trim((string)($_POST['bowl_ratio'][$bIndex] ?? ''));
            $storedBowlName = $build_bowl_storage_name($bName, $mixRatio);
            
            $materials = $_POST['material_id'][$bIndex] ?? [];
            $amounts = $_POST['amount_g'][$bIndex] ?? [];
            
            for ($i = 0; $i < count($materials); $i++) {
                if (!empty($materials[$i]) && (!empty($amounts[$i]) || $amounts[$i] === '0')) {
                    $f_stmt->execute([$visit_id, $materials[$i], $amounts[$i], $storedBowlName]);
                }
            }
        }

        // Homecare products
        $product_ids = $_POST['product_ids'] ?? [];
        $product_prices = $_POST['product_prices'] ?? [];
        $product_amounts = $_POST['product_amounts'] ?? [];
        $p_stmt = $pdo->prepare("INSERT INTO visit_products (visit_id, product_id, price_sold, amount) VALUES (?, ?, ?, ?)");
        
        for ($i = 0; $i < count($product_ids); $i++) {
            if (!empty($product_ids[$i])) {
                $p_stmt->execute([
                    $visit_id, 
                    $product_ids[$i], 
                    (int)($product_prices[$i] ?? 0),
                    (int)($product_amounts[$i] ?? 1)
                ]);
            }
        }
        
        $pdo->commit();
        $_SESSION['msg'] = "Změny receptur i produktů byly uloženy.";
    } catch(Exception $e) {
        $pdo->rollBack();
        $_SESSION['msg'] = "Chyba při úpravě návštěvy: " . $e->getMessage();
    }

    header("Location: index.php?client_id=" . $client_id);
    exit;
}
header("Location: index.php");
exit;
